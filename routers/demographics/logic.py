...
async def calculate_demographics_score(req: ReqDemographics) -> Dict[str, Any]:
    ...
    results = await Database.fetch(SqlObject.fetch_population_data_query, req.lat, req.lng, req.radius)
    
    # Calculate demographics score with detailed breakdown
    score_data = calculate_score_from_results(results, req.target_age, req.sex_preference)
    
    return score_data


def calculate_score_from_results(results: list, target_age: int, sex_preference: str = None) -> Dict[str, Any]:
    ...
    if sex_preference == "male":
        target_population = sum((row["Male_Population"] or 0) for row in results)
        weighted_median_age = sum((row["Median_Age_Male"] or 0) * (row["Male_Population"] or 0) for row in results) / target_population
        sex_desc = "male population"
    elif sex_preference == "female":
        target_population = sum((row["Female_Population"] or 0) for row in results)
        weighted_median_age = sum((row["Median_Age_Female"] or 0) * (row["Female_Population"] or 0) for row in results) / target_population
        sex_desc = "female population"
    else:
        # No sex preference - use total population
        target_population = sum((row["Population_Count"] or 0) for row in results)
        weighted_median_age = sum((row["Median_Age_Total"] or 0) * (row["Population_Count"] or 0) for row in results) / target_population
        sex_desc = "total population"
    
    avg_density = sum((row["Population_Density_KM2"] or 0) for row in results) / len(results)
    ...
    age_quality = "excellent" if age_proximity_factor > 0.8 else "good" if age_proximity_factor > 0.6 else "moderate" if age_proximity_factor > 0.3 else "poor"
    density_quality = "high" if density_factor > 0.7 else "moderate" if density_factor > 0.4 else "low"
    size_quality = "large" if population_factor > 0.7 else "medium" if population_factor > 0.4 else "small"
