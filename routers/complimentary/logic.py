...
async def calculate_complementary_score(req: ReqComplementary) -> Dict[str, Any]:
    ...
    complementary_data = await fetch_complementary_data_from_external_api(
        req.lat, req.lng, req.radius, req.complementary_business_categories, auth_token
    )
    
    # Calculate complementary score with detailed breakdown
    score_data = calculate_score_from_complementary_data(complementary_data, req.target_num_per_category)
    
    return score_data


async def fetch_complementary_data_from_external_api(lat: float, lng: float, radius: int, categories: List[str], auth_token: str) -> List[Dict]:
...
def calculate_score_from_complementary_data(complementary_data: List[Dict], target_num_per_category: int) -> Dict[str, Any]:
    ...
    for business in complementary_data:
        category = business.get("types", ["unknown"])[0] if business.get("types") else "unknown"
    ...
    coverage_factor = min(1.0, target_categories / 5)  # Assuming 5 is ideal category coverage
    ...
    coverage_quality = "excellent" if coverage_factor > 0.8 else "good" if coverage_factor > 0.6 else "limited"
    balance_quality = "well-balanced" if balance_factor > 0.7 else "adequate" if balance_factor > 0.4 else "uneven"
    
    top_categories = sorted(category_counts.items(), key=lambda x: x[1], reverse=True)[:3]
    top_cat_desc = ", ".join([f"{cat}: {count}" for cat, count in top_categories])
