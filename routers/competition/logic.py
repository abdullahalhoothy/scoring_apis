...
async def calculate_competition_score(req: ReqCompetition) -> Dict[str, Any]:
    ...
    competition_data = await fetch_competition_data_from_external_api(
        req.lat, req.lng, req.radius, req.competition_business_categories, auth_token
    )
    
    # Calculate competition score with detailed breakdown
    score_data = calculate_score_from_competition_data(competition_data, req.target_num_per_category)
    
    return score_data


async def fetch_competition_data_from_external_api(lat: float, lng: float, radius: int, categories: List[str], auth_token: str) -> List[Dict]:
...
def calculate_score_from_competition_data(competition_data: List[Dict], target_num_per_category: int) -> Dict[str, Any]:
    ...
    avg_per_category = total_competitors / len(category_counts) if category_counts else 0
    ...
    competition_level = "low" if competition_factor > 0.7 else "moderate" if competition_factor > 0.4 else "high"
    category_distribution = "well-distributed" if category_factor > 0.6 else "concentrated" if category_factor > 0.3 else "oversaturated"
